...
class LPR():
    ...
    def SimpleRecognizePlateByE2E(self,image):
        t0=time.time()
        #images = self.detectPlateRough(image,image.shape[0],top_bottom_padding_rate=0.005)
...
#        w=rect[2]
#        h=rect[3]
#        plate=self.cropImage(image, (int(x), int(y), int(w), int(h)))
        image=image[0:72,0:225]
        #cv2.imshow('rs',image)
        #cv2.waitKey(0)
